...
/** Loads and renders the leaderboard into #player-stats-list */
async function loadPlayerStats() {
  ...
  try {
    ...
    // Exclude users whose screenName starts with "testUser"
...
// Expose globally so the nav controller can call it
window.loadPlayerStats = loadPlayerStats;

document.addEventListener('DOMContentLoaded', function() {
  // Keep hidden-button stubs for backward compatibility with any tests
  const showBtn = document.getElementById('show-player-stats-btn');
  const backBtn = document.getElementById('back-to-main-btn');
  if (showBtn) showBtn.addEventListener('click', loadPlayerStats);
